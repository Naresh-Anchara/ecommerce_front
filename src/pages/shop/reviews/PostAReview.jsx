...
const PostAReview = ({isModalOpen,handleClose}) => {
    ...
    const handleSubmit = async (e) =>{
        ...
        if (!rating || !comment.trim()){
          alert("All information is required. Please provide a rating and a comment.");
          return;
        }
      const newComment = {
        comment: comment,
      ...
      try {
       const response = await postReview(newComment).unwrap();
        ...
        alert("comment posted failed please try again");
      }
      handleClose();
    }
  return (
    ...
    ${isModalOpen ? 'block' : 'hidden'}`}>
      ...
      {/* <textarea
        ...
        rows={4} // Use curly braces for numbers to avoid string coercion.
    ...
    onChange={(e) => setComment(e.target.value)}
    rows={4}
    className="w-full border border-gray-300 rounded-md mb-4 p-3 focus:outline-none focus:ring-2 focus:ring-blue-500"
